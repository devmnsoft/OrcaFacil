...
function localWrite(name, data){ const list=JSON.parse(localStorage.getItem(localKey(name))||'[]'); list.unshift({...data, createdAt:new Date().toISOString()}); localStorage.setItem(localKey(name), JSON.stringify(list.slice(0,500))); }
...
async function write(collectionName, data){ if (isDemo()) { localWrite(collectionName, data); return data.id; } await setDoc(doc(collection(db, collectionName), data.id), data); return data.id; }
...
export const MonitoringService = {
  ...
  async trackEvent(type, payload={}){
    ...
    try { await write('systemEvents', data); } catch (err) { if(dev) console.error('trackEvent failed', err); }
    return id;
  },
  async trackError(error, context={}){
    ...
    try { await write('systemErrors', data); if(severity==='critical') await queueTelegramNotification(context.type||'CRITICAL_ERROR','🚨 OrçaFácil - Erro crítico',`${data.message}\nUsuário: ${data.userEmail||'-'}\nPágina: ${location.pathname}`, { ...data, demo:isDemo() }, 'critical'); } catch (err) { if(dev) console.error('trackError failed', err); }
    return id;
  },
  async audit(action, entityType, entityId, before=null, after=null, metadata={}){
    ...
    try { await write('auditLogs', data); } catch (err) { if(dev) console.error('audit failed', err); }
    return id;
  },
  installGlobalHandlers(toast){
    window.addEventListener('error', e=>{ this.trackError(e.error||e.message,{ file:e.filename, line:e.lineno, column:e.colno, type:'GLOBAL_ERROR' }); if(toast) toast('Ocorreu um erro inesperado. Nossa equipe técnica foi notificada.','error'); });
    window.addEventListener('unhandledrejection', e=>{ this.trackError(e.reason||'Promise rejeitada',{ type:'UNHANDLED_REJECTION' }); if(toast) toast('Não foi possível concluir a operação. Tente novamente.','error'); });
  }
};
