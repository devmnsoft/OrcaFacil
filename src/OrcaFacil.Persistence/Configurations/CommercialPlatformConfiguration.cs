...
public sealed class BillingInvoiceConfiguration : IEntityTypeConfiguration<BillingInvoice> { public void Configure(EntityTypeBuilder<BillingInvoice> b) { b.ToTable("billing_invoices"); b.ConfigureBase(); b.HasIndex(x => x.AccountId); b.HasIndex(x => x.Status); b.HasIndex(x => x.ExternalReference).IsUnique(); b.HasIndex(x => x.DueAt); b.Property(x => x.Status).HasConversion<string>(); b.Property(x => x.Cycle).HasConversion<string>(); b.Property(x => x.Subtotal).HasPrecision(18,2); b.Property(x => x.DiscountAmount).HasPrecision(18,2); b.Property(x => x.Amount).HasPrecision(18,2); b.Property(x => x.PaidAmount).HasPrecision(18,2); b.Property(x => x.InvoiceNumber).HasMaxLength(60); b.Property(x => x.Currency).HasMaxLength(3); b.Property(x => x.CancellationReason).HasMaxLength(500); } }
