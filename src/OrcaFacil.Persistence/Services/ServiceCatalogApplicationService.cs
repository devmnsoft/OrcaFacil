...
public sealed class ServiceCatalogApplicationService(OrcaFacilDbContext db, ICurrentAccountService account, ICurrentUserService user) : IServiceCatalogApplicationService
{
    public async Task<ServiceCatalogPage?> ListAsync(ServiceCatalogQuery request, CancellationToken ct = default)
    {
        ...
        var query = all;
        if (!string.IsNullOrWhiteSpace(request.Search)) { var term = $"%{request.Search.Trim()}%"; query = query.Where(x => EF.Functions.ILike(x.Name, term) || (x.Code != null && EF.Functions.ILike(x.Code, term)) || (x.Description != null && EF.Functions.ILike(x.Description, term))); }
    ...
    private static void Apply(ServiceCatalogItem x, ServiceCatalogInput i) { x.Name = i.Name.Trim(); x.Code = string.IsNullOrWhiteSpace(i.Code) ? null : i.Code.Trim(); x.Description = i.Description?.Trim(); x.CategoryId = i.CategoryId; x.UnitCode = i.UnitCode; x.StandardPrice = i.StandardPrice; x.EstimatedCost = i.EstimatedCost; x.SuggestedDurationMinutes = i.SuggestedDurationMinutes; x.InternalNotes = i.InternalNotes?.Trim(); x.IsFavorite = i.IsFavorite; x.IsActive = i.IsActive; }
}
