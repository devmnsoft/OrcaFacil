...
public static class PublicApiV1
{
    ...
    private static async Task<IResult> CreateClient(CreateClientRequest request, HttpContext http, ClaimsPrincipal user, OrcaFacilDbContext db, CancellationToken ct)
    {
        ...
        var accountId=Account(user);var keyId=Guid.Parse(user.FindFirstValue("api_key_id")!);var keyHash=Hash(header.ToString());var requestHash=Hash(System.Text.Json.JsonSerializer.Serialize(request));
    ...
    private static Guid Account(ClaimsPrincipal user)=>Guid.Parse(user.FindFirstValue("account_id")!);
