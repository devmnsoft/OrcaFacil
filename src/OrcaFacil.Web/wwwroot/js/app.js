(function(){
  ...
  window.confirmAction=(message='Confirmar ação?')=>window.confirm(message);
  window.setButtonLoading=btn=>{if(!btn||btn.dataset.loadingSet)return;btn.dataset.loadingSet='true';btn.dataset.originalText=btn.innerHTML;btn.innerHTML='Aguarde...';btn.disabled=true};
  window.calculateDocumentTotals=()=>{let subtotal=0;document.querySelectorAll('.item').forEach(row=>{const q=parseFloat((row.querySelector('.qty')?.value||'0').replace(',','.'));const p=parseFloat((row.querySelector('.price')?.value||'0').replace(',','.'));const d=parseFloat((row.querySelector('.discount')?.value||'0').replace(',','.'));const total=Math.max(0,(q*p)-d);subtotal+=total;const out=row.querySelector('[data-item-total]');if(out)out.textContent=money(total)});document.querySelectorAll('[data-subtotal]').forEach(x=>x.textContent=money(subtotal));const disc=parseFloat((document.querySelector('[data-document-discount]')?.value||'0').replace(',','.'));document.querySelectorAll('[data-total]').forEach(x=>x.textContent=money(Math.max(0,subtotal-disc)))};
  window.addDocumentItem=()=>{const items=document.querySelector('#items');const first=items?.querySelector('.item');if(!items||!first)return;const clone=first.cloneNode(true);const index=items.querySelectorAll('.item').length;clone.querySelectorAll('input').forEach(input=>{input.value='';input.name=input.name.replace(/Items\[\d+\]/,`Items[${index}]`)});items.appendChild(clone);calculateDocumentTotals()};
