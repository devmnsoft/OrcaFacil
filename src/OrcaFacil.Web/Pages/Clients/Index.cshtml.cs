...
public sealed class IndexModel(IClientWorkspaceService workspace) : PageModel
{
    ...
    public string Mask(BrazilianDocumentType? type, string? number) =>
        MaskDocument(type, number);

    public static string MaskDocument(BrazilianDocumentType? type, string? number) =>
        BrazilianDocument.Mask(type, number);
}
