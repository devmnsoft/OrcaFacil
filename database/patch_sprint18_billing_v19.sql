BEGIN;
-- V1.9 is additive: no customer, subscription, invoice, or payment history is removed.
...
CREATE TABLE IF NOT EXISTS orcafacil.subscription_change_requests (id uuid PRIMARY KEY, account_id uuid NOT NULL, requested_by_user_id uuid NOT NULL, current_plan_id uuid, requested_plan_id uuid, request_type varchar(40) NOT NULL, status varchar(30) NOT NULL, reason varchar(1000) NOT NULL, admin_notes varchar(1000), reviewed_at timestamptz, completed_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz, is_deleted boolean NOT NULL DEFAULT false);
...
CREATE TABLE IF NOT EXISTS orcafacil.plan_addons (id uuid PRIMARY KEY, code varchar(80) NOT NULL UNIQUE, name text NOT NULL, description text NOT NULL, price_monthly numeric(18,2) NOT NULL CHECK(price_monthly >= 0), price_annual numeric(18,2) NOT NULL CHECK(price_annual >= 0), limit_type text NOT NULL, limit_increment bigint NOT NULL CHECK(limit_increment > 0), is_active boolean NOT NULL DEFAULT true, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz, is_deleted boolean NOT NULL DEFAULT false);
